    ...
    /* Indica que o jogo está em execução */
		...
		while (running) {
				...
				if (enemy1_states[i] == ACTIVE) {
					...
					if (enemy1_Y[i] > GameLib.HEIGHT + 10) {
						...
						if (currentTime > enemy1_nextShoot[i] && enemy1_Y[i] < player_Y) {
							...
							int free = ao.findFreeIndex(e_projectile_states);
				...
				if (enemy2_states[i] == ACTIVE) {
					...
					if (enemy2_X[i] < -10 || enemy2_X[i] > GameLib.WIDTH + 10) {
						...
						if (shootNow) {
							...
							int[] freeArray = ao.findFreeIndex(e_projectile_states, angles.length);
			...
			if (currentTime > nextEnemy1) {
				...
				int free = ao.findFreeIndex(enemy1_states);
			...
			if (currentTime > nextEnemy2) {
				...
				int free = ao.findFreeIndex(enemy2_states);
			...
			if (player_state == ACTIVE) {
				...
				if (GameLib.iskeyPressed(GameLib.KEY_CONTROL)) {
					...
					if (currentTime > player_nextShot) {
						...
						int free = ao.findFreeIndex(projectile_states);
			...
			for (int i = 0; i < projectile_states.length; i++) {
				...
				if (projectile_states[i] == ACTIVE) {
					...
					GameLib.drawLine(projectile_X[i] - 1, projectile_Y[i] - 3, projectile_X[i] - 1,
							projectile_Y[i] + 3);
					GameLib.drawLine(projectile_X[i] + 1, projectile_Y[i] - 3, projectile_X[i] + 1,
							projectile_Y[i] + 3);
				}
			...
			for (int i = 0; i < enemy1_states.length; i++) {
				...
				if (enemy1_states[i] == EXPLODING) {
					...
					double alpha = (currentTime - enemy1_explosion_start[i])
							/ (enemy1_explosion_end[i] - enemy1_explosion_start[i]);
			...
			for (int i = 0; i < enemy2_states.length; i++) {
				...
				if (enemy2_states[i] == EXPLODING) {
					...
					double alpha = (currentTime - enemy2_explosion_start[i])
							/ (enemy2_explosion_end[i] - enemy2_explosion_start[i]);
